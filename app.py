...
def take_test(test_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT test_name, test_timer FROM Test WHERE test_id = %s",
            (test_id,),
        )
        ...
        cursor.execute(
            ...
            SELECT q.question_id,
                   q.question_content,
                   q.question_type,
                   COALESCE(tq.custom_score, q.max_score) AS max_score,
                   c.choice_id,
                   c.choice_content
            FROM Test_Question tq
            JOIN Question q ON tq.question_id = q.question_id
            LEFT JOIN Choice c ON c.question_id = q.question_id
            WHERE tq.test_id = %s
            ORDER BY q.question_id ASC, c.choice_id ASC
            """,
            (test_id,),
        )
        ...
        for row in question_rows:
            if current_question is None or current_question['question_id'] != row['question_id']:
                current_question = {
    ...
    return render_template('take_test.html', test_id=test_id, test_info=test_info, questions=questions)
...
def submit_test(test_id):
    ...
    timer_seconds = request.form.get('timer_seconds')
    timer_value = None
    try:
        if timer_seconds is not None and timer_seconds != '':
            timer_value = int(timer_seconds)
    except ValueError:
        timer_value = None
    ...
    try:
        ...
        attempt_index = attempt_row['next_index'] if attempt_row else 1

        cursor.execute(
            "INSERT INTO Attempt (attempt_index, end_time, timer, test_id, student_id) VALUES (%s, NOW(), %s, %s, %s)",
            (attempt_index, timer_value, test_id, student_id),
        )
        attempt_id = cursor.lastrowid

        cursor.execute(
            "SELECT q.question_id FROM Test_Question tq JOIN Question q ON tq.question_id = q.question_id WHERE tq.test_id = %s",
            (test_id,),
        )
        ...
        for row in questions:
            question_id = row['question_id']
            selected_choice = request.form.get(f'question_{question_id}')
            if selected_choice:
                try:
...
def test_result(test_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT attempt_index, start_time, end_time, timer, score FROM Attempt WHERE test_id = %s AND student_id = %s ORDER BY attempt_index DESC",
            (test_id, student_id),
        )
        attempts = cursor.fetchall()

        cursor.execute("SELECT fn_MaxScore_Student_Test(%s, %s) AS best_score", (student_id, test_id))
        score_row = cursor.fetchone()
        best_score = score_row['best_score'] if score_row and score_row['best_score'] is not None else 0
    except mysql.connector.Error as e:
        flash(f"Database error: {e}", 'danger')
        return redirect(url_for('student_dashboard'))
    finally:
    ...
    return render_template('test_result.html', test_id=test_id, test_info=test_info, attempts=attempts, best_score=best_score)
